...
std::unordered_map<uint16_t, GameItem> Registry::wallItems;
...
// ── Walls ────────────────────────────────────────────────────────────
    wallItems[0] = {0, "None",      ItemType::Wall, ToolType::None, 0, 1, false, 1.0f, TileBreakType::None,   0,  0.0f};
    wallItems[1] = {1, "StoneWall", ItemType::Wall, ToolType::None, 1, 1, false, 1.0f, TileBreakType::Hammer, 50, 0.5f};
...
// ── Tools ────────────────────────────────────────────────────────────
    ...
    std::cout << "OK: Registry initialized with " << items.size() << " items, " << wallItems.size() << " walls.\n";
}
...
const GameItem& Registry::getWall(uint16_t id) {
    auto it = wallItems.find(id);
    if (it == wallItems.end()) {
        throw std::runtime_error("Registry: unknown wall id: " + std::to_string(id));
    }
    return it->second;
}
