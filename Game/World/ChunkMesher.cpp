...
namespace ChunkMesher {
    void mesh(const Chunk& chunk, std::vector<Vertex>& vertices, std::vector<uint32_t>& indices, int chunkSize, std::vector<QuadSpec>& quads) {
        ...
        for (int ty = 0; ty < chunkSize; ++ty) {        // y outer
            for (int tx = 0; tx < chunkSize; ++tx) {    // x inner
                const Tile& tile = chunk.tiles[tx + ty * chunkSize]; // sequential
                if (!tile.isActive && tile.wallId == 0) continue;

                const float wx = static_cast<float>(chunk.chunkX * chunkSize + tx)
                                 * Constants::TileSize;
                const float wy = static_cast<float>(chunk.chunkY * chunkSize + ty)
                                 * Constants::TileSize;
